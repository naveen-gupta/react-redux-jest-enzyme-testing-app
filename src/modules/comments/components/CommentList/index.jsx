...
const CommentList = (props) => {
  ...
  if (comments.length > 0) {
    if (filterCommentCriteria) {
      ...
      if (filteredComments.length > 0) commentList = filteredComments.sort(sortByModifiedAt).map((c) => <Comment {...props} commentDetails={c} />);
      else commentList = noCommentsFoundMessage;
    } else {
